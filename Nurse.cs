...
namespace Hospital_Escape
{
    ...
    public class Nurse : subject
    {
        ...
        public bool checkup(int id)
        {
            if (checkInventory("bpmachine") || checkInventory("heartmachine")||checkInventory("stethescope"))
            {
                bool success = this.currentRoom.tryCheckPatient(id);
                if (success)
                {
        ...
        public bool save(int id)
        {
            ...
            {
                bool success = this.currentRoom.trySavePatient(id);
                if (success)
                {
                    Console.WriteLine(" You saved a patient");
                    NotificationCenter.Instance.PostNotification(new Notification("patient saved", this));
