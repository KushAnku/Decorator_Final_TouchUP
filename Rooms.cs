    ...
    // key - to get in the particular room.
{
    ...
    public class Rooms : Observer
    {
        ...
        {
            Patient apatient = new Patient(_patients.Count + 1);
            apatient.room = this;
            _patients.Add(apatient);
        }

        public bool trySavePatient(int id)
        {
            id = id - 1;
            ...
            else if(_patients[id].state == State.DYING)
            {
                _patients[id].state = State.FINE;
                return true;
            }
            return false;
        }

        public bool tryCheckPatient(int id)
        {
            id--;
            if (_patients.Count <= 0)
            {
            ...
            else if(_patients[id].state == State.CHECK)
            {
                _patients[id].state = State.FINE ;
                return true;
            }
